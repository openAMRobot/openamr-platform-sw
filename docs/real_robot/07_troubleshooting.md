...
| `sim:=false requires an explicit map` | no `map:=` passed | pass `map:=/home/botshare/maps/piece_actuelle.yaml` |
| Camera "no cameras available" | forgot to source `~/camera_ws` (RPi libcamera fork) | source the camera overlay ([`01`](01_bringup.md) §0) |
